...
func (me *DvTest) GetData() test.Table {
	return test.Table{
		test.NewFixture(&test.Fixture{
			...
			In:    "1.27.3-beta!",
			Out: test.Out{
				parseDv: test.Args{Fail: true, Want: "pre-release in '1.27.3-beta!' is not valid semver"},
			},
		}),
		test.NewFixture(&test.Fixture{
			Label: "Full version plus invalid Pre-release",
...
func (me *DvTest) MakeNewObject(f *test.Fixture) (obj interface{}, err error) {
	var dv *projectcfg.DottedVersion
	dv = projectcfg.NewDottedVersion()
	err = dv.Parse(f.In)
	return dv, err
}

func (me *DvTest) GetOutput(f *test.Fixture) (got string) {
	dv := f.Obj.(*projectcfg.DottedVersion)
	switch f.Name {
	case parseDv:
		got = dv.GetVersion()
